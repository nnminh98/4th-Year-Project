...
from RoutingControllers import RandomRouting, Dijkstra
...
class Network(object):
    ...
    def add_nodes(self, nodes):
        """
        Add a set of nodes into the network
        nodes = ["n1", "n2", "n3", "n4"]
    ...
    def add_link(self, src, dst, rate, qlimit, monitor_rate, propagation_delay):
        ...
        if self.contains_node(src) and self.contains_node(dst):
            ...
            print("Graph does not contain node " + str(src.id) + " or node " + str(dst.id))
    ...
    def set_routing_algorithm(self, controller):
        for node in self.nodes.values():
            node.set_routing_algo(controller=controller)
...
class Node(object):
    ...
    def __init__(self, id, env, gen):
        ...
        self.switch_port = SwitchPort(env=env, rate=500, qlimit=64, limit_bytes=False, id=self.id)
    ...
    def add_connection(self, dst_node, rate, qlimit, monitor_rate, propagation_delay, bidirectional=True):
        ...
        new_link = Link(self.env, id=link_id, cost=propagation_delay, dst=dst_node, src=self.id)
        self.routes[link_id] = new_link

        if bidirectional:
            new_link_reverse = dst_node.add_connection(self, rate, qlimit, monitor_rate, propagation_delay, bidirectional=False)
